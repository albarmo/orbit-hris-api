...
func NewPayrollItemRepository(db *gorm.DB) PayrollItemRepository { return &payrollItemRepository{db: db} }
...
func (r *payrollItemRepository) FindAll(ctx context.Context) ([]entities.PayrollItem, error) {
    ...
    if err := r.db.WithContext(ctx).Find(&list).Error; err != nil { return nil, err }
    return list, nil
}
...
func (r *payrollItemRepository) FindByID(ctx context.Context, id uuid.UUID) (*entities.PayrollItem, error) {
    ...
    if err := r.db.WithContext(ctx).First(&p, "id = ?", id).Error; err != nil { return nil, err }
    return &p, nil
}
