...
type payrollItemService struct{ repo repository.PayrollItemRepository }

func NewPayrollItemService(r repository.PayrollItemRepository) PayrollItemService { return &payrollItemService{repo: r} }
...
func (s *payrollItemService) Create(ctx context.Context, req dto.PayrollItemCreateRequest) (dto.PayrollItemResponse, error) {
    ...
    if err := s.repo.Create(ctx, p); err != nil { return dto.PayrollItemResponse{}, err }
...
func (s *payrollItemService) FindAll(ctx context.Context) ([]dto.PayrollItemResponse, error) {
    ...
    for _, p := range list { out = append(out, dto.PayrollItemResponse{ID: p.ID.String(), PayrollID: p.PayrollID.String(), Name: p.Name, Amount: p.Amount}) }
    return out, nil
}

func (s *payrollItemService) FindByID(ctx context.Context, id uuid.UUID) (dto.PayrollItemResponse, error) {
    p, err := s.repo.FindByID(ctx, id)
    if err != nil { return dto.PayrollItemResponse{}, err }
    return dto.PayrollItemResponse{ID: p.ID.String(), PayrollID: p.PayrollID.String(), Name: p.Name, Amount: p.Amount}, nil
}
...
func (s *payrollItemService) Update(ctx context.Context, id uuid.UUID, req dto.PayrollItemUpdateRequest) (dto.PayrollItemResponse, error) {
    ...
    if req.Amount != nil { p.Amount = *req.Amount }
    if err := s.repo.Update(ctx, p); err != nil { return dto.PayrollItemResponse{}, err }
