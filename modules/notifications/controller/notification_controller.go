...
func NewNotificationController(s service.NotificationService) NotificationController { return &notificationController{svc: s} }
...
func (c *notificationController) GetByUser(ctx *gin.Context) {
    ...
    if err != nil { ctx.JSON(http.StatusBadRequest, gin.H{"error":"invalid user id"}); return }
...
func (c *notificationController) Delete(ctx *gin.Context) {
    id, err := uuid.Parse(ctx.Param("id"))
    if err != nil { ctx.JSON(http.StatusBadRequest, gin.H{"error":"invalid id"}); return }
    if err := c.svc.Delete(ctx.Request.Context(), id); err != nil { ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
