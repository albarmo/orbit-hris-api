...
func PermissionSeeder(db *gorm.DB) error {
	...
	var listData []entities.Permission
	if err := json.Unmarshal(jsonData, &listData); err != nil {
		return err
	}

	hasTable := db.Migrator().HasTable(&entities.Permission{})
	if !hasTable {
		if err := db.Migrator().CreateTable(&entities.Permission{}); err != nil {
			return err
		}
	}

	for _, data := range listData {
		var existingData entities.Permission
